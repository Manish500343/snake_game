    ...
    def draw(self,surface):
        ...
        for row in range(SQUARES):
            for col in range(SQUARES):
                if counter%2==0:
                    pygame.draw.rect(surface,BG2,
                                     (col*PIXELS,row*PIXELS,PIXELS,PIXELS))
                if col!=SQUARES-1:
                    counter+=1
...
class Collision:
    ...
    def between_snake_and_walls(self,snake):
        if snake.headX<0 or snake.headY>WIDTH-PIXELS or snake.headY<0 or snake.headY>HEIGHT-PIXELS:
            return True
        return False

    def between_snake_and_body(self,snake):
...
async def main():
    ...
    while True:
        ...
        if collision.between_snake_and_body(snake):
            snake.die()
            apple.spawn()
            score.reset()
